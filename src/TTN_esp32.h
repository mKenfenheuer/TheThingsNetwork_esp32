...
public:
	...
	///
	bool join(
		const char* appEui, const char* appKey, int8_t retries = -1, uint32_t retryDelay = 10000);
	...
	/// @param retryDelay The delay in milliseconds between each retry
	///
	bool join(const char* devEui, const char* appEui, const char* appKey, int8_t retries = -1,
	...
	size_t getDevEui(byte* buf, bool hardwareEui = false);
	///
...
///
	...
	///
	/// Get the Appkey as String
	/// @return A string containing the AppKey
	///
	String getAppKey();
